...
# Function used to verify if the provided user exists on the local system
function checkUser($name){

    $checkStatus = Get-LocalUser | Where-Object { $_.name -ilike $name } -ErrorAction SilentlyContinue
    if ($true -eq $checkStatus){
        return $true
    }
    elseif ($null -eq $checkStatus) {
        return $false
    }
}
